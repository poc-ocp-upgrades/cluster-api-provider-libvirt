...
func createCommand() *cobra.Command {
	...
	return &cobra.Command{Use: "create", Short: "Create machine instance for specified cluster", RunE: func(cmd *cobra.Command, args []string) error {
		...
		cluster, machine, userData, err := utils.ReadClusterResources(cmd.Flag("cluster").Value.String(), cmd.Flag("machine").Value.String(), cmd.Flag("userdata").Value.String())
		if err != nil {
			return err
		}
...
func bootstrapCommand() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "bootstrap", Short: "Bootstrap kubernetes cluster with kubeadm", RunE: func(cmd *cobra.Command, args []string) error {
		...
		f := framework.Framework{SSH: &framework.SSHConfig{Key: masterguestpk, User: "fedora"}}
		glog.Info("Collecting master kubeconfig")
		config, err := f.GetMasterMachineRestConfig(masterMachine, lcw)
		if err != nil {
			return err
		}
		clusterFramework, err := framework.NewFrameworkFromConfig(config, &framework.SSHConfig{Key: masterguestpk, User: "fedora"})
		if err != nil {
			return err
		}
