...
type httpImage struct{ url *url.URL }
...
func (i *httpImage) size() (uint64, error) {
	...
	if response.StatusCode != 200 {
		return 0, fmt.Errorf("Error accessing remote resource: %s - %s", i.url.String(), response.Status)
	}
	length, err := strconv.Atoi(response.Header.Get("Content-Length"))
	if err != nil {
		err = fmt.Errorf("Error while getting Content-Length of %q: %v - got %s", i.url.String(), err, response.Header.Get("Content-Length"))
		return 0, err
	}
...
type localImage struct{ path string }
