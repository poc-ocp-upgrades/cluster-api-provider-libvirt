...
func (a *Actuator) createVolumeAndDomain(machine *machinev1.Machine, machineProviderConfig *providerconfigv1.LibvirtMachineProviderConfig, client libvirtclient.Client) (*libvirt.Domain, error) {
	...
	if err := client.CreateVolume(libvirtclient.CreateVolumeInput{VolumeName: domainName, BaseVolumeName: machineProviderConfig.Volume.BaseVolumeID, VolumeFormat: "qcow2"}); err != nil {
		return nil, a.handleMachineError(machine, apierrors.CreateMachine("error creating volume %v", err), createEventAction)
	}
	if err := client.CreateDomain(libvirtclient.CreateDomainInput{DomainName: domainName, IgnKey: machineProviderConfig.IgnKey, Ignition: machineProviderConfig.Ignition, VolumeName: domainName, CloudInitVolumeName: cloudInitVolumeName(domainName), IgnitionVolumeName: ignitionVolumeName(domainName), NetworkInterfaceName: machineProviderConfig.NetworkInterfaceName, NetworkInterfaceAddress: machineProviderConfig.NetworkInterfaceAddress, AddressRange: a.cidrOffset, HostName: domainName, Autostart: machineProviderConfig.Autostart, DomainMemory: machineProviderConfig.DomainMemory, DomainVcpu: machineProviderConfig.DomainVcpu, CloudInit: machineProviderConfig.CloudInit, KubeClient: a.kubeClient, MachineNamespace: machine.Namespace}); err != nil {
